...
- **更新下载改为「GitHub 加速镜像优先，GitHub 直链与官网下载双双兜底」**。下载前先对
  gh-proxy.com / ghfast.top / ghproxy.net 三个加速镜像并发测速，按实测速度从快到慢
  依次尝试；都不行再试 GitHub 原站直链，最后才回落到官网的服务器下载地址。
  起因：不少用户反馈直连 GitHub 拉包极慢甚至超时，实测**查询能通、下载必死**
  （API 请求 0.8 秒就回来，100MB 的包直接 ReadTimeout）。
- **检查更新的元数据同样走加速镜像**：Release 附件 update.json 经镜像读取，原站次之，
  官网服务器第三，查不到才退回 GitHub API。注意「查询」与「下载」能用的镜像**并不重合**
  —— 下载三家都行，但代理 api.github.com 只有 gh-proxy.com 支持（另两家返回 403），
...
- 官网下载地址保持从自有服务器下载，不受本次改动影响。
- 说明：本次改动只影响「检查更新 / 下载新版本」这条链路，发票识别、去重、汇总
  等主功能与 v5.1.2 完全一致。旧版本客户端要升到本版才会用上新的更新逻辑。

2026-09-22  v5.1.2
- **更新下载改为「GitHub 加速镜像优先，自有服务器兜底」** —— 下载前先对
  gh-proxy.com / ghfast.top / ghproxy.net 三个加速镜像做并发测速，按实测速度
  从快到慢依次尝试；GitHub 原站与自有服务器直链保留在候选列表里兜底。
- **为什么改**：实测裸网直连 GitHub 只有 3.7 KB/s（基本等于不可用），而公共加速镜像
  可达 439 KB/s；镜像之间也能差近 10 倍（439 / 223 / 46 KB/s），所以不能死按固定
  顺序试，必须测速择优。自有服务器受带宽上限约束（实测封顶约 445 KB/s），
  已无速度优势，让出主源位置改为兜底。
- **检查更新同样优先走加速镜像**（update.json 经镜像读取），自有服务器
  /updates/qr.json 退为兜底。
- 探测失败的源不会被丢弃，只排到最后再试一次 —— 任何镜像全挂都还能更新。
...
def _start_update_flow(root: tk.Tk, download_urls, version: tuple, sha256: str = ""):
    ...
    perform_update 会再展开成各加速镜像并测速择优，最终顺序为
    加速镜像 → GitHub 原站 → 官网服务器），依次尝试直到成功，
