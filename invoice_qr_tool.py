...
4. 可选任务完成后打开文件夹；
5. 可选将下载的 PDF 转换为图片（长边 2000px，短边自适应）；
...
4. 可选：处理完成后自动打开文件夹。
5. 可选：将下载的 PDF 转为图片（长边 2000px，短边自适应），保存到「PDF/图片」。
6. 可选：处理完成后汇总发票（生成 Excel）。对「PDF」文件夹内所有发票 PDF，
   提取「交款人 / 票据号码 / 开票日期 / 金额合计（小写）/ 医保统筹基金支付」，
   汇总为「PDF/发票汇总_YYYYMMDD_HHMMSS.xlsx」；金额与统筹列为数字格式（可直接求和），
   并新增「是否重复」列（同一票据号码出现≥2次则标记“是”）；表格右侧统计区新增：
   票据张数 / 合计总金额 / 合计总统筹金额 / 可赔付金额 / 重复票据金额合计 / 重复票据统筹合计。
...
- 勾选汇总             → PDF/发票汇总_YYYYMMDD_HHMMSS.xlsx（金额/统筹为数字格式，含「是否重复」列与右侧重复汇总）
- 勾选汇总             → PDF/发票汇总_YYYYMMDD_HHMMSS.xlsx（金额/统筹为数字格式，右侧含统计区）
...
class InvoiceQrToolApp:
    ...
    def _build_ui(self):
        ...
        ttk.Button(
            frame_action, text="检查更新", command=self._check_update
